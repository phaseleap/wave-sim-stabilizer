...
def simulate_with_metrics(sim: WaveSimStabilizer, apply_suppression=True):
    psi = sim.initial_condition().astype(complex)
    psi_t = np.copy(psi)
...
if __name__ == "__main__":
    sim = WaveSimStabilizer(g=2.5, theta=0.1, lambda_=1.2)
    max_w, var_w, Q_w = simulate_with_metrics(sim, apply_suppression=True)
    max_wo, var_wo, Q_wo = simulate_with_metrics(sim, apply_suppression=False)
